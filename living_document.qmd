...
Add instructions for creating and linking a .bib file to a Quarto document #14
...
Use `@key` to cite a source, where `key` is the BibTeX ID from the `.bib` file.

```markdown
Statistical NLP gained traction in the 1990s [@manning1999].
```

To suppress the author name (for example, in parenthetical form), add a minus sign:

```markdown
Many advances were based on earlier probabilistic methods [-@manning1999].
```
